-- Step 1: Emergency Data Cleanup - Fixed Version 2
...
DROP FUNCTION IF EXISTS public.geocode_and_create_service_location();

-- Create a clean, simple function for service location creation
...
BEGIN
  ...
  IF NEW.service_street IS NOT NULL AND NEW.service_city IS NOT NULL AND NEW.service_state IS NOT NULL THEN
    ...
    -- Create service location with pending geocoding status
...
-- Create function to reset failed geocoding
...
BEGIN
  -- Reset failed geocoding statuses to pending
