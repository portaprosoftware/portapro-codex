-- Safe, idempotent migration for incidents and spill kits without performed_at indexes
...
CREATE INDEX IF NOT EXISTS idx_spill_incident_reports_created_at ON public.spill_incident_reports(created_at);
...
CREATE INDEX IF NOT EXISTS idx_vehicle_spill_kit_checks_created_at ON public.vehicle_spill_kit_checks(created_at);
...
DO $$ BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'decon_logs' AND column_name = 'created_at'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_decon_logs_created_at ON public.decon_logs(created_at);
  END IF;
END $$;