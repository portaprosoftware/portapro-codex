-- Fix the get_product_availability_enhanced function to use individual attribute columns
...
DECLARE
  ...
  filter_winterized boolean;
BEGIN
  -- Extract filter values if provided
  IF filter_attributes IS NOT NULL THEN
    filter_color := filter_attributes->>'color';
    filter_size := filter_attributes->>'size';
    filter_material := filter_attributes->>'material';
    filter_winterized := (filter_attributes->>'winterized')::boolean;
  END IF;
  ...
  -- Get individual items (tracked units only) with proper attribute filtering
  SELECT array_agg(
    jsonb_build_object(
      'item_id', pi.id,
  ...
  AND (filter_color IS NULL OR pi.color = filter_color)
  AND (filter_size IS NULL OR pi.size = filter_size)
  AND (filter_material IS NULL OR pi.material = filter_material)
  AND (filter_winterized IS NULL OR pi.winterized = filter_winterized);
  
  -- Generate daily breakdown for date range
  date_iter := start_date;
  WHILE date_iter <= end_date LOOP
    -- Count available tracked units for this date with attribute filtering
    SELECT COUNT(*)
    INTO day_available
    FROM public.product_items pi
    WHERE pi.product_id = product_type_id
    ...
    -- Get conflicts (active assignments) for this date
    SELECT array_agg(
      jsonb_build_object(
        'assignment_id', ea.id,
