...
-- First, delete the duplicate 'New Customers' segments, keeping only one
...
-- Delete duplicates of 'Active Customers' if any, keeping only one
DELETE FROM public.customer_segments 
...
BEGIN
  -- Get the rule set for this segment
  SELECT rule_set INTO segment_rule_set 
  FROM public.customer_segments 
  WHERE id = segment_id;
  ...
  -- Get segment name to determine how to count
  DECLARE
    segment_name TEXT;
  BEGIN
    SELECT name INTO segment_name 
    FROM public.customer_segments 
    WHERE id = segment_id;
