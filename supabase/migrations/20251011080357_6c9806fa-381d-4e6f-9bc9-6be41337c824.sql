-- Fix get_vehicle_summary fuel section to use unified_fuel_consumption table
  ...
  SELECT json_build_object(
    ...
    'fuel', json_build_object(
      ...
      'avg_mpg_30d', (
        SELECT ROUND(AVG(
          CASE 
            WHEN gallons > 0 AND odometer_reading IS NOT NULL THEN
              (odometer_reading - LAG(odometer_reading) OVER (ORDER BY fuel_date)) / gallons
            ELSE NULL
          END
        )::numeric, 2)
        FROM unified_fuel_consumption
