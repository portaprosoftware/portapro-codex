...
WHERE nsp.nspname = 'public'
  AND rel.relname = 'consumables'
  AND con.contype = 'c';

-- Then create a fixed migration with proper categories
-- Consumable notification settings
CREATE TABLE IF NOT EXISTS public.consumable_notification_settings (
...
-- Seed consumables with common items using allowed categories
...
('Toilet Paper Rolls', '2-ply toilet paper for portable units', 'cleaning', 'SUP-001', 0.85, 1.50, 200, 50, '{"vendor": "PaperPlus", "order_code": "PP-2PLY-IND"}', true, NULL),
('Paper Towels', 'Absorbent paper towels for handwash stations', 'cleaning', 'SUP-002', 1.25, 2.25, 100, 25, '{"vendor": "PaperPlus", "order_code": "PP-TOWEL-200"}', true, NULL),
...
INSERT INTO public.notification_logs (user_id, notification_type, title, body, data, related_entity_type, related_entity_id) 
SELECT 
  '00000000-0000-0000-0000-000000000000', 
  'consumable_low_stock', 
  'Low Stock Alert: Tank Treatment', 
  'Tank Treatment is running low with only 15 units remaining (threshold: 5)', 
  '{"consumable_name": "Tank Treatment", "current_stock": 15, "threshold": 5}', 
  'consumable', 
  c.id
FROM public.consumables c 
WHERE c.name = 'Tank Treatment' 
LIMIT 1
