-- SQL functions for Fleet Maintenance Module Phases 3-5
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_maintenance_records_scheduled_date ON public.maintenance_records(scheduled_date) WHERE status NOT IN ('completed', 'cancelled');
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_maintenance_records_technician_status ON public.maintenance_records(technician_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_maintenance_records_vehicle_type ON public.maintenance_records(vehicle_id, maintenance_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_maintenance_records_completed_date ON public.maintenance_records(completed_date) WHERE status = 'completed';