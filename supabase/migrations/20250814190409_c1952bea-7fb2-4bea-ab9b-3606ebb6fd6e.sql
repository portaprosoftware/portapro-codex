-- Fix maintenance count in unified stock function
...
BEGIN
  ...
  -- Get individual item counts including maintenance
  SELECT 
    COUNT(*) as total,
    COUNT(*) FILTER (WHERE status = 'available') as available,
    COUNT(*) FILTER (WHERE status = 'maintenance') as maintenance
  INTO individual_count, individual_available, maintenance_count
  ...
  result := jsonb_build_object(
    ...
    'individual_items', jsonb_build_object(
      ...
      'assigned', individual_count - individual_available - maintenance_count,
