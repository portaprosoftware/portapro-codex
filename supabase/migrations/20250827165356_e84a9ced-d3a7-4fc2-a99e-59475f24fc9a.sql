-- Implement enhanced availability function using variations (product_item_attributes + product_properties)
-- Replaces any previous version that referenced a non-existent attributes column
...
BEGIN
  ...
  matching_items AS (
    ...
    HAVING COALESCE((
      ...
      FROM filters f
      JOIN public.product_item_attributes pia ON pia.product_item_id = pi.id
      JOIN public.product_properties pp ON pp.id = pia.property_id
      WHERE pp.attribute_name = f.attribute_name AND pp.attribute_value = ANY(f.values)
  ...
  attrs AS (
    SELECT pia.product_item_id AS item_id,
           jsonb_object_agg(pp.attribute_name, pp.attribute_value) AS attrs_map
    FROM public.product_item_attributes pia
    JOIN public.product_properties pp ON pp.id = pia.property_id
    WHERE pia.product_item_id IN (SELECT id FROM ti)
    GROUP BY pia.product_item_id
  )
  ...
  -- Overall availability is the minimum daily total_available
