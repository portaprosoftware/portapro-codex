...
-- 2) Make sure there is an index to quickly find invitations by clerk id (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3) Create or replace RPC to sync Clerk profile without requiring a UUID id from client
...
BEGIN
  ...
  IF existing_id IS NULL THEN
    -- Insert new profile, generate UUID for internal id
