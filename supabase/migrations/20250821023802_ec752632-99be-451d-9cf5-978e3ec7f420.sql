-- Update get_product_availability_enhanced to work for ALL products like the product overview
...
BEGIN
  ...
  -- Return complete breakdown for all products
  RETURN jsonb_build_object(
    'available', available_stock,
    'total', product_record.stock_total,
    'method', CASE 
      WHEN individual_count > 0 THEN 'hybrid_tracking'
      ELSE 'bulk_only'
    END,
    'individual_items', COALESCE(items, '[]'::jsonb),
    'bulk_assigned', bulk_assigned,
