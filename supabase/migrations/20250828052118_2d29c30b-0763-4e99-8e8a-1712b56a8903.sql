-- Create a new availability function that only considers tracked units
...
BEGIN
  ...
  -- Get individual items (tracked units only)
  SELECT array_agg(
    jsonb_build_object(
      'item_id', pi.id,
      ...
      'attributes', COALESCE(pi.attributes, '{}'::jsonb)
    )
  )
  ...
  AND (
    filter_attributes IS NULL OR 
    (pi.attributes @> filter_attributes)
  );
