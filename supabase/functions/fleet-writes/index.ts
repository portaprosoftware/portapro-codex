...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create_fuel_log') {
      const { data, error } = await supabase.from('fuel_logs').insert([payload]).select('id').single();
    ...
    if (action === 'update_fuel_log') {
      if (!payload?.id) throw new Error('Missing id');
      const { id, ...update } = payload;
      const { error } = await supabase.from('fuel_logs').update(update).eq('id', id);
      if (error) throw error;
    ...
    if (action === 'delete_fuel_log') {
      if (!payload?.id) throw new Error('Missing id');
      const { error } = await supabase.from('fuel_logs').delete().eq('id', payload.id);
      if (error) throw error;
      return new Response(JSON.stringify({ success: true }), {
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
