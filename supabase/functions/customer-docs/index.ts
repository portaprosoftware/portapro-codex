...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create_signed_upload') {
      ...
      const { data, error } = await supabase.storage.from('customer-documents').createSignedUploadUrl(payload.path);
    ...
    if (action === 'create_signed_url') {
      ...
      const { data, error } = await supabase.storage.from('customer-documents').createSignedUrl(payload.path, expiresIn);
      if (error) throw error;
      return new Response(JSON.stringify({ success: true, data }), {
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
    ...
    if (action === 'delete_object') {
      if (!payload?.path) throw new Error('Missing path');
      const { error } = await supabase.storage.from('customer-documents').remove([payload.path]);
      if (error) throw error;
