...
interface EditStorageSiteModalProps {
...
export function EditStorageSiteModal({ site, open, onOpenChange, onClose }: EditStorageSiteModalProps) {
  ...
  useEffect(() => {
    if (site) {
      setFormData({
        ...
        custom_street: site.custom_street || "",
        custom_street2: site.custom_street2 || "",
        custom_city: site.custom_city || "",
        custom_state: site.custom_state || "",
        custom_zip: site.custom_zip || "",
        gps_latitude: site.gps_coordinates ? site.gps_coordinates.y.toString() : "",
        gps_longitude: site.gps_coordinates ? site.gps_coordinates.x.toString() : "",
  ...
  const { data: existingDefault } = useQuery({
    ...
    queryFn: async () => {
      ...
      // It's okay if no default exists
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      payload.custom_street = null;
      payload.custom_street2 = null;
      payload.custom_city = null;
      payload.custom_state = null;
      payload.custom_zip = null;
    ...
    onError: (error) => {
      toast.error("Failed to update storage site: " + error.message);
    }
  });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button
              ...
              className="flex-1"
            >
              {updateMutation.isPending ? "Updating..." : "Update Storage Site"}
            </Button>
          </div>
        </form>
      </DialogContent>
