          ...
          <Tabs value={activeTab} onValueChange={(value: string) => setActiveTab(value as "details" | "workorders" | "update")} className="w-full">
            ...
            <TabsContent value="update" className="mt-6">
              ...
              <div className="space-y-5">
                ...
                {/* Display Work Orders with Nested Progress Updates */}
                <div className="bg-white border rounded-xl p-4">
                  <h4 className="font-medium mb-4">Work Orders & Updates</h4>
                  <div className="space-y-3">
                    {workOrders.length === 0 ? (
                      <div className="text-center py-8 text-muted-foreground">
                        No work orders yet. Create one in the Work Orders tab.
                      </div>
                    ) : (
                      workOrders.map((workOrder: any) => {
                        const progressUpdates = getProgressUpdatesForWorkOrder(workOrder.id);
                        const isExpanded = expandedWorkOrders[workOrder.id] ?? (workOrder.status !== 'completed');
                        
                        return (
                          <div key={workOrder.id} className="border-l-4 border-l-primary rounded-lg border p-4">
                            {/* Work Order Header */}
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <Badge variant="secondary" className="capitalize">
                                    {workOrder.update_type?.replace("_", " ") || 'Work Order'}
                                  </Badge>
                                  {workOrder.status && (
                                    <Select
                            ...
                            <p className="text-sm mb-3">{workOrder.description}</p>

                            {(workOrder.technician_name || workOrder.labor_hours || workOrder.parts_used) && (
                              <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground mb-3">
                                {workOrder.technician_name && (
                                  <div>
                                    <span className="font-medium">Technician:</span> {workOrder.technician_name}
                                  </div>
                                )}
                                {workOrder.labor_hours && (
                                  <div>
                                    <span className="font-medium">Labor:</span> {workOrder.labor_hours} hrs
                                  </div>
                                )}
                                {workOrder.parts_used && Array.isArray(workOrder.parts_used) && workOrder.parts_used.length > 0 && (
                                  <div className="col-span-2">
                                    <span className="font-medium">Parts:</span> {workOrder.parts_used.join(', ')}
                                  </div>
                                )}
                              </div>
                            )}
                            ...
                            {workOrder.cost_amount > 0 && (
                              <div className="flex items-center gap-2 text-sm font-medium mb-3">
                                ...
                                Total Cost: ${workOrder.cost_amount.toFixed(2)}
                              </div>
                            )}

                            {workOrder.attachments && Array.isArray(workOrder.attachments) && workOrder.attachments.filter((att: any) => att.type === 'photo').length > 0 && (
                              <div className="grid grid-cols-4 gap-2 mb-3">
                                {workOrder.attachments.filter((att: any) => att.type === 'photo').map((photo: any, idx: number) => (
                                  <img
                                    key={idx}
                                    src={photo.url}
                                    alt={photo.caption || `Work order photo ${idx + 1}`}
                                    className="w-full h-20 object-cover rounded cursor-pointer hover:opacity-80 transition-opacity"
                                    onClick={() => {
                                      const photos = workOrder.attachments.filter((att: any) => att.type === 'photo');
                                      setSelectedPhotos(photos);
                            ...
                            {progressUpdates.length > 0 && (
                              <div className="mt-4 pt-4 border-t">
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => setExpandedWorkOrders(prev => ({ ...prev, [workOrder.id]: !isExpanded }))}
                                  className="mb-2 w-full justify-between hover:bg-muted/50"
                                >
                                  ...
                                  <div className="space-y-2 ml-4 pl-4 border-l-2 border-muted">
                                    {progressUpdates.map((update: any) => (
                                      <div key={update.id} className="bg-muted/30 rounded-lg p-3">
                                        ...
                                        {update.attachments && Array.isArray(update.attachments) && update.attachments.filter((att: any) => att.type === 'photo').length > 0 && (
                                          <div className="grid grid-cols-4 gap-2 mt-2">
                                            {update.attachments.filter((att: any) => att.type === 'photo').map((photo: any, idx: number) => (
                                              <img
                                                key={idx}
                                                src={photo.url}
                                                alt={photo.caption || `Update photo ${idx + 1}`}
