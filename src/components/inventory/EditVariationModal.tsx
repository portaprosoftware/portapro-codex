...
export const EditVariationModal: React.FC<EditVariationModalProps> = ({
  isOpen,
  onClose,
  productId,
  variation,
  ...
  // Populate form when variation prop changes
  useEffect(() => {
    if (variation) {
      setVariationName(variation.name);
      setVariationValues([...variation.values]);
      setIsRequired(variation.required);
    }
  }, [variation]);
  ...
  const updateVariationMutation = useMutation({
    mutationFn: async () => {
      ...
      // First, delete all existing entries for this variation
      const { error: deleteError } = await supabase
        .from('product_properties')
        .delete()
        .eq('product_id', productId)
        .eq('attribute_name', variation?.name);
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md bg-white">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the variation type and its available values.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={() => updateVariationMutation.mutate()}
            disabled={updateVariationMutation.isPending || !variationName.trim() || variationValues.length === 0}
            className="bg-blue-600 hover:bg-blue-700"
          >
            {updateVariationMutation.isPending ? "Updating..." : "Update Variation"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
