...
const getCurrentTimePosition = () => {
  ...
  if (currentHour < 8) {
    ...
    adjustedPosition = (totalHoursFromStart / 2) * (1 / ALL_TIMELINE_HOURS.length);
  } else if (currentHour >= 17) {
    // After 5pm column (last column)
    const lastColumnStart = (ALL_TIMELINE_HOURS.length - 1) / ALL_TIMELINE_HOURS.length;
    const positionInLastColumn = ((currentHour - 17) + minutesAsHours) / 3; // 3 hours in after 5pm
    adjustedPosition = lastColumnStart + (positionInLastColumn * (1 / ALL_TIMELINE_HOURS.length));
  } else {
    // Regular hourly columns (8am-4pm)
    const columnIndex = currentHour - 7; // 8am = column 1, 9am = column 2, etc.
    adjustedPosition = columnIndex / ALL_TIMELINE_HOURS.length + (minutesAsHours / 60) * (1 / ALL_TIMELINE_HOURS.length);
  }
  
  return adjustedPosition * 100;
};

export const TimelineGrid: React.FC<{ timeFilter?: 'all' | 'morning' | 'afternoon' }> = ({ timeFilter = 'all' }) => {
  const hours = timeFilter === 'morning' ? MORNING_HOURS : timeFilter === 'afternoon' ? AFTERNOON_HOURS : ALL_TIMELINE_HOURS;
  return (
