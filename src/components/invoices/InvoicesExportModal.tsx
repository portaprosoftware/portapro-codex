...
export function InvoicesExportModal({ isOpen, onClose }: InvoicesExportModalProps) {
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (filters.format === 'csv') {
        ...
        downloadCSV(csvContent, `invoices_export_${format(new Date(), 'yyyy-MM-dd')}.csv`);
      } else {
      ...
      toast({
        ...
        description: `Invoices data exported successfully as ${filters.format.toUpperCase()}.`
      });
      onClose();
    },
    onError: (error) => {
      toast({
        title: "Export Failed",
        description: "Failed to export invoices data. Please try again.",
        variant: "destructive"
      });
  ...
  const generateCSVContent = (records: any[]) => {
    ...
    const rows = records.map(record => {
      ...
      if (filters.includeItems) {
        const itemsCount = record.invoice_items?.length || 0;
        const itemsDetail = record.invoice_items?.map((item: any) => 
      ...
      if (filters.includePayments) {
        ...
        const paymentsDetail = record.payments?.map((payment: any) => 
          `$${payment.amount} (${payment.status}) - ${payment.created_at ? format(new Date(payment.created_at), 'MMM dd, yyyy') : ''}`
        ).join('; ') || '';
        
        baseRow.push(paymentsCount.toString(), paymentsDetail);
      }
  ...
  const generateHTMLReport = (records: any[]) => {
    const totalValue = records.reduce((sum, record) => sum + (record.amount || 0), 0);
    const totalPaid = records.reduce((sum, record) => {
      const paid = record.payments?.reduce((paySum: number, payment: any) => 
      ...
      <body>
        ...
        <div class="summary">
          ...
          <div class="summary-grid">
            ...
            <div class="summary-item">
              <h3>Total Paid</h3>
              <p style="font-size: 24px; color: #059669;">$${totalPaid.toLocaleString()}</p>
            </div>
            <div class="summary-item">
              <h3>Outstanding</h3>
              <p style="font-size: 24px; color: #dc2626;">$${totalOutstanding.toLocaleString()}</p>
            </div>
          </div>
          ...
          <tbody>
            ${records.map(record => {
              const totalPaid = record.payments?.reduce((sum: number, payment: any) => 
                payment.status === 'completed' ? sum + payment.amount : sum, 0) || 0;
              const balanceDue = (record.amount || 0) - totalPaid;
              const isOverdue = record.due_date && new Date(record.due_date) < new Date() && record.status === 'unpaid';
              ...
              return `
                <tr ${isOverdue ? 'class="overdue"' : ''}>
                  ...
                  <td>${record.due_date ? format(new Date(record.due_date), 'MMM dd, yyyy') : ''}</td>
                  <td>${record.status || ''}</td>
                  <td class="amount">$${(record.amount || 0).toLocaleString()}</td>
                  <td class="amount paid">$${totalPaid.toLocaleString()}</td>
                  <td class="amount ${balanceDue > 0 ? 'overdue' : 'paid'}">$${balanceDue.toLocaleString()}</td>
                </tr>
              `;
            }).join('')}
          </tbody>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Export invoices data for accounting and reporting. Select your filters and format below.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFilters(prev => ({ ...prev, includePayments: !!checked }))}
                />
                <Label htmlFor="include-payments" className="text-sm font-normal">
                  Include payment history (CSV only)
                </Label>
              </div>
            </div>
          </div>
